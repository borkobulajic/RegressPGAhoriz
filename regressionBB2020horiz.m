function [ScallingCoefficients] = regressionBB2020horiz(MRSGSL,HorizPGA,DistanceBound,SourceToSiteDistance,saNo)

% regressionBB2020horiz  - Multiple linear regression analysis for the horizontal PGA values
%
% Inputs:
...
% 2) variable HorizPGA with twice as 203 rows and 1 column. The first 203 rows 
%   consist of the horizontal PGA values in one orthogonal horizontal direction 
%   and the next 203 rows consist of the corresponding horizontal PGA values in
%   the other orthogonal horizontal direction.
...
%   saNo is the number of the equations. In that case the variable HorizPGA
...
sa(find(abs(MRSGSL(:,1))<=DistanceBound)+size(MRSGSL,1),:)];
...
for j=1:n_sa
	...
	for i=1:n_R0values
		...
		%EPICENTRAL DISTANCE
		Cs=[ones(size(thedata,1),1) thedata(:,3) log10((thedata(:,1).^2+(R0values(i))^2).^0.5) thedata(:,5) thedata(:,6) thedata(:,4);ones(size(thedata,1),1) thedata(:,3) log10((thedata(:,1).^2+(R0values(i))^2).^0.5) thedata(:,5) thedata(:,6) thedata(:,4)];
		elseif SourceToSiteDistance==2
		%HYPOCENTRAL DISTANCE
		Cs=[ones(size(thedata,1),1) thedata(:,3) log10((thedata(:,1).^2+(R0values(i))^2+thedata(:,2).^2).^0.5) thedata(:,5) thedata(:,6) thedata(:,4);ones(size(thedata,1),1) thedata(:,3) log10((thedata(:,1).^2+(R0values(i))^2+thedata(:,2).^2).^0.5) thedata(:,5) thedata(:,6) thedata(:,4)];
		end    
    ...
    RESULTShor1=bcoeffS(Is,:);
    RESULTShor=[RESULTShor; RESULTShor1];
    RESIDUALShor1=RESIDUALS(Is,:);
    RESIDUALShor=[RESIDUALShor; RESIDUALShor1];
